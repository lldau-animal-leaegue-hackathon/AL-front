...
- [x] Step 7(사용자 지시 2026-08-20 "다음 후보군 진행해") — **지식 소비 화면 + 정규화**.
      - `src/lib/ingredientNames.ts`: 별칭 확장("클레이(카올린·벤토나이트)" → 3별칭) +
        포함 매칭. 실측: "아스코빅애씨드"가 "비타민씨(아스코빅애씨드)" 지식을 찾는다.
      - `GET /api/knowledge/ingredient` · `GET /api/knowledge/concerns` — **AI 없는
        0원 경로**(실측 300~400ms). 매칭은 앱 코드(지식이 수백 행 규모).
      - `IngredientDetailModal`(공용): 고민별 도움·타입별 주의·시너지·충돌·그 성분이 든
        제품. **파트너 칩을 누르면 모달 안에서 그 성분으로 이동**(사전 걷기).
        DESIGN.md 모달 스펙(중앙·32px 패딩·radius-xl)을 처음으로 준수한 모달.
      - `ConcernDictionary`("내 고민" 탭): 자주 찾는 고민 8종 → 성분 → 상세.
        펼쳐야 불러온다. 리포트 탭 칩들도 상세 모달로 연결(중첩 버튼 자리는 제외).
      - `IngredientChip` 을 `src/components/` 로 승격(onSelect 로 버튼/스팬 겸용).
