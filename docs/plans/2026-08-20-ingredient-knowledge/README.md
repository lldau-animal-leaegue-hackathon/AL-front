...
- [x] Step 0 — 완료 2026-08-20. 헤딩 wonder 삽입 + DataState 다크 호버 수정.
      커밋: `0dc9f19`.
- [x] Step 1 — 완료 2026-08-20. 003 서버 적용, concern 캐시(가드보다 먼저 조회),
      profile UPSERT 의 조건부 리셋(`IF(wonder = ?, …)` — `VALUES()` 함수 인자 금지).
      검증 7/7 PASS: **첫 호출 30,433ms → 캐시 히트 44ms(691배)**, 가용 시간만
      재저장 시 캐시 유지, wonder 변경 시 NULL 리셋. 커밋: `d615055`.
- [x] Step 1.5(추가 결정) — 완료 2026-08-20. "주의 없음(빈 배열)"도 저장해 성분알림
      무한 재시도 종료. 빈 배열은 **첫 기록(NULL)일 때만** 허용 — 기존 값 덮기는
      409(데이터 손실 가드 유지). 커밋: `0b91351`.
- [x] Step 2 — 완료 2026-08-20. interactions.ts(정본)·/api/ai/report·004 서버 적용·
      지문(shelfFingerprint). 실호출 12/12 PASS — **충돌: 레티놀+아스코빅애씨드,
      시너지: 나이아신아마이드+레티놀**(사용자가 예로 든 조합을 AI 가 실제로 잡음),
      캐시 히트 47ms, 선반 변경 시 재생성 57초. 규칙 1·4·5 위반은 경고 로그+항목
      버림으로 방어. 커밋: `ad1190f`.
- [x] Step 3+4 — 완료 2026-08-20. 하위 탭 3분할(ScanTabs 밑줄 패턴)·IngredientChip
      (DESIGN.md 칩 스펙 첫 구현)·IngredientProducts 공용 승격·추천 성분 펼침 연결.
      **교훈: AI SWR 캐시의 무효화는 `mutate(key, undefined)` 가 아니라 키 교체로** —
      자동 재검증이 꺼져 있어 열려 있는 구독자가 영영 로딩에 갇힌다. 키에 선반
      서명(djb2)을 넣어 products 갱신이 곧 키 교체가 되게 했다.
      브라우저 실측: 탭 전환 중 /ai/report 재요청 0건, 생성 표시(경과초+장기 안내),
      다크에서 칩 토큰 쌍 정상. 커밋: `997096b`.
- [x] Step 5 — 완료 2026-08-20. `design-debt.md` 백로그 작성(보통 5·낮음 7·문서 문제).
- [x] Step 6(사용자 지시 2026-08-20) — **공유 지식 테이블**. Q2-B 로 미뤄 뒀던 것을
      지시("다양한 시나리오의 성분들을 디비에 저장해줘")로 앞당겼다.
      005 마이그레이션(`ingredient_pairs`·`ingredient_effects`, 서버 적용) +
      시드 100건(시너지 20·충돌 20·고민 8시나리오 39·타입 4종 21 — 헤드리스 Claude
      4회 생성, 세션이 전수 검수: 모순 쌍 0·진단 표현 0) +
      **수확 훅**: /api/ai/report 가 새 분석을 만들 때마다 쌍·타입별 주의를
      INSERT IGNORE — 쓸수록 지식이 쌓인다. 시드는 `db/seeds/001_*.sql` 로 커밋
      (INSERT IGNORE 라 재실행 안전 — 실측: 재적용 후 100행 불변).
      ~~다음 후보: 이 테이블을 읽는 화면, 성분명 정규화~~ → Step 7 로 진행(사용자 지시).
- [x] Step 7(사용자 지시 2026-08-20 "다음 후보군 진행해") — **지식 소비 화면 + 정규화**. - `src/lib/ingredientNames.ts`: 별칭 확장("클레이(카올린·벤토나이트)" → 3별칭) +
      포함 매칭. 실측: "아스코빅애씨드"가 "비타민씨(아스코빅애씨드)" 지식을 찾는다. - `GET /api/knowledge/ingredient` · `GET /api/knowledge/concerns` — **AI 없는
      0원 경로**(실측 300~400ms). 매칭은 앱 코드(지식이 수백 행 규모). - `IngredientDetailModal`(공용): 고민별 도움·타입별 주의·시너지·충돌·그 성분이 든
      제품. **파트너 칩을 누르면 모달 안에서 그 성분으로 이동**(사전 걷기).
      DESIGN.md 모달 스펙(중앙·32px 패딩·radius-xl)을 처음으로 준수한 모달. - `ConcernDictionary`("내 고민" 탭): 자주 찾는 고민 8종 → 성분 → 상세.
      펼쳐야 불러온다. 리포트 탭 칩들도 상세 모달로 연결(중첩 버튼 자리는 제외). - `IngredientChip` 을 `src/components/` 로 승격(onSelect 로 버튼/스팬 겸용).
      검증: API 7/7(정규화 포함), 브라우저 전 흐름(펼침→칩→모달→파트너 이동→닫기),
      check·build 통과. **미착수로 남긴 것**: 리포트 시너지 탭의 AI 전 DB 폴백
      (성분 상세 모달이 같은 지식을 이미 보여줘 우선순위가 내려갔다),
      제품 상세 모달(PopularProductModal) 성분 → 지식 연결(모달 중첩 문제).
