...
### ✅ 실측 결과 (2026-08-18, 배포 전 일괄 검증)

**정적**: `npm run check` · `npm run build` 통과. 빌드 라우트 표에서 `/api/*` 8종이 전부
`ƒ`(동적)이고 Proxy 가 동작한다.

**API 31개 항목 전부 통과** — 빈 상태 / 캐시 헤더(`private, no-store` + `Vary: Cookie`) /
이관 / **이관 멱등성**(2회 호출해도 제품·루틴·기록이 그대로) / 데이터 손실 방어 /
입력 검증(길이·개수·날짜) / 사용자 격리 / **AI 동시 실행 가드(2번째 요청 429)**.

**UI (browser-prober, 390px)**:

| 항목                        | 결과                                                                       |
| --------------------------- | -------------------------------------------------------------------------- |
| 빈 상태 문구                | 5개 경로 전부 정확. "불러오지 못했어요"(에러)로 오지 않는다                |
| `NaN`·`undefined`·`0%` 노출 | 0건                                                                        |
| 가로 스크롤 · 탭바 겹침     | 0건 (`main` 의 `padding-bottom: 100px` 이 fixed nav 를 피한다)             |
| **DB 끊김 시 화면**         | 통째로 비는 경로 **0** — 전부 "불러오지 못했어요 + 다시 시도"가 뜬다       |
| **에러가 빈 상태로 위장**   | 0건 (`/routine/new` 도 "제품을 등록해 주세요"가 아니라 에러 문구를 띄운다) |
| **503 응답의 정보 노출**    | **없음.** body 는 일반 문구뿐이고 접속 정보·스택이 섞이지 않는다           |
| 로딩 표시                   | 섹션 단위로 "불러오는 중…"이 뜨고 화면 전체가 비지 않는다                  |
| 이관                        | 옛 localStorage 데이터가 서버로 넘어가고 로컬 4개 키가 사라진다            |

**검증이 잡은 결함 2건 (둘 다 수정 후 재확인):**

1. **MariaDB 에서 `VALUES()` 를 함수 인자로 넘기면 값이 전달되지 않는다.**
   `JSON_LENGTH(VALUES(ingredients))` 가 죽어 같은 제품 재등록이 503 이었다.
   ⚠️ **더 위험한 건 이게 "방어 성공"으로 보였다는 점이다** — 성분·썸네일이 안 덮인 이유가
   방어가 아니라 **요청 자체가 실패해서**였다. 성공 여부를 함께 보지 않으면 통과로 읽힌다.
2. **`WeekStrip` 이 기록 없이도 지난 요일을 완료로 표시했다.** "오늘 이전은 완료"라는
   백엔드 이전 시절의 임시 규칙이 남아 있었다. 근거 없는 진행률을 만들지 않는다는 Q10 원칙을
   정면으로 위배하는 자리였다 — 실제 `runs` 에서만 완료가 나오도록 고쳤다.

📌 **검증 시 함정**: 브라우저에 옛 localStorage 데이터가 남아 있으면 **접속하자마자 이관이 돌아**
서버가 더 이상 비어 있지 않다. "빈 상태"를 보려면 DB 를 비우는 것만으로 부족하고
**브라우저의 localStorage 와 `al_uid` 쿠키까지** 지워야 한다(1차 검증이 이걸로 무효가 됐다).
...
3. **프론트 접속용 서브도메인** — ~~프록시 종류 미확인~~ → **nginx 로 확인**(2026-08-18).
   컨테이너가 아니라 **호스트에서 직접** 돌고 설정은 `/etc/nginx/{sites-enabled,conf.d}` 에 있다.
   착수 시 확인할 것: 기존 사이트 설정을 어떻게 나누는지, TLS 발급 수단(certbot 여부).
   ⚠️ **HTTPS 가 붙어야 쿠키가 정상 동작한다** — `proxy.ts` 가 실제 프로토콜로 `secure` 를
   정하므로, 프록시 뒤에서는 `X-Forwarded-Proto` 를 넘겨 주어야 한다. 안 넘기면 HTTPS 인데도
   `secure` 없이 쿠키가 구워진다.
