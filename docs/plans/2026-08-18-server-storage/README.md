...
> ✅ **DB 개설 완료(2026-08-18)** — `ThisisServer` 에 `al-front-db` 컨테이너가 떴고
> `001_init.sql` 이 적용됐다(테이블 8개, `schema_migrations` = `001`).
> 접속 정보는 로컬 `.env.local` 에 있다. **Step 3(쿼리 계층 + Route Handler 4종)까지 완료 —
> 실호출로 검증했다. 남은 것은 Step 4(클라이언트 데이터 계층)부터다.**
>
...
`CREATE DATABASE` 를 litedeck 만으로는 못 친다. **실제 작업은 `ssh ThisisServer` 직결로 했다**
(`~/.ssh/config` 에 등록돼 있어 Bash 에서 바로 붙는다). litedeck 은 현황 조회에 썼다.

**서버 실측** — ⛔ **이 레포는 퍼블릭이다.** 서버 주소·OS 버전·타 서비스 구성처럼
정찰에 쓰이는 정보는 여기 적지 않는다. 접속 수단은 로컬 `~/.ssh/config` 의 `ThisisServer`
...
| 공유 상태    | 다른 서비스가 다수 함께 돈다 — **포트·리소스를 뺏지 않는지 확인하고 붙일 것**(3306·3307 사용 중) |

### 3. ~~Step 3 부터 이어서~~ → **Step 4 부터 이어서** (2026-08-18 갱신)

Step 3(쿼리 계층 + Route Handler 4종)까지 끝났고 **실호출로 검증됐다.**
다음은 **Step 4 — 클라이언트 데이터 계층**이다: `swr` 설치 → `useStored` 를 서버 조회로 교체 →
화면의 `ready` 2상태를 **로딩·에러·재시도**로 넓힌다.
...
컨테이너를 띄웠다**(사용자 결정). 그 컨테이너는 `thisis` 서비스용이고 compose 관리도 아니라
CI/CD 로 묶을 수 없다.

| 항목      | 값                                                                                            |
| --------- | --------------------------------------------------------------------------------------------- |
| 컨테이너  | `al-front-db` — `mariadb:latest`(=11.3.2), **`pull_policy: missing`** 으로 기존 이미지 재사용 |
| 포트      | **`127.0.0.1:3308` 만.** 인터넷 노출 0 (3306·3307 과 다르다)                                  |
| 접속      | **SSH 터널**(사용자 결정): `ssh -L 3308:127.0.0.1:3308 ThisisServer`                          |
...
- [x] **Step 5 앞부분 — DB 연결 + 마이그레이션 실행** — 위 "DB 개설" 로 완료.
- [ ] **Step 5 나머지 — 앱에서 실제 연결.** 엔드투엔드는 Step 3·4 가 있어야 돈다.
...
1. **작업 폴더 CI/CD 연결** — 서버에 이미 `2taeyeop/*` 이미지를 쓰는 파이프라인이 있다
   (`animal-league-*`, `gts`, `react-app`). 같은 방식을 따르는지 먼저 확인할 것.
