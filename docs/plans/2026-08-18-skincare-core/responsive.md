...
- [x] R-Step 3 — 매직 넘버 제거 — **완료 2026-08-18.** 4개 페이지 컨테이너 + 수행 화면. 홈·루틴·스캔은
  ...
  **✅ 수행 화면(마지막 2줄)도 해결 — `--step-actions-h` 신설(2026-08-18)**

  `routine/[routineId]/[step]/page.module.css`의 `8px … 180px`(`:8`·`:119`)는 회피 대상이
  탭바가 아니라 **`StepActions` 고정 액션바**라 `--page-pb`로 치환할 수 없었다.
  `--page-pb`는 `--nav-h`(76px) 기반이고 **lg 이상에서 40px로 줄지만, 액션바는 그 구간에도
  계속 떠 있기** 때문이다. 그래서 전용 토큰을 만들었다.
  ...
  넓은 화면에서 커지는 이유: `.primary`가 `--text-body-lg`(=`clamp(16px,1.5vw,18px)`)를 쓴다.
  → **`--step-actions-h: 156px`** (실측 최대 + 폰트 폴백 여유 3px). 데스크톱 padding 오버라이드는 삭제.

  ⚠️ **처음엔 152px로 잡았다가 실측에서 0.89px 부족이 나와 올렸다.** 겹침은 없었지만
  (`+24px` 버퍼가 흡수) 토큰이 사실과 어긋나면 안 된다. 계산만 믿었으면 틀린 채로 남았다.

  ⚠️ **`.subtle` 터치 타깃 수정과 값이 연동된다** — R-Step 5에서 `.subtle`에 `min-height: 44px`를
  넣으면 `.secondary` 행이 24px→44px가 되어 바 전체가 **약 20px 높아진다.** 두 작업은 파일이
  다른데 값이 흐르므로, 액션바 높이를 재는 시점이 `.subtle` 수정 전이면 토큰이 20px 모자란다.

  **최종 실측(수정 후)**: 하단 여유 390px `+26.27` / 768px `+28.97` / 1440px `+110.42`. **겹침 0건.**
...
- [x] R-Step 4 — 그리드 — **완료 2026-08-18. 단 R9(컨테이너 쿼리)는 의도적으로 구현하지 않았다.**

  **R2 해결** — 홈 `.narrow`가 768px에서 실폭 약 219px(1컬럼 36.9px + gap 24)라 카드가 뭉갰다.
  md(768–1023)는 `.wide`→`span 12`·`.narrow`→`span 6`(약 460px), lg(1024–)에서 목업의 8/4를 복원.

  **⛔ R9 컨테이너 쿼리 — 넣지 않았다.** 근거:
  - R9의 논거는 _"뷰포트는 넓은데 카드는 좁은 조합에서 어긋난다"_ 는 **가정**이고,
    이 문서에도 코드에도 **실제로 깨진 사례가 없다.** 문서의 `@container` 예시는 가상 코드다.
  - 그 원인(219px 카드)을 R4의 span 정책이 **직접 제거**한다. 전제가 사라졌다.
  - 소비하는 `@container` 규칙이 0인데 `container-type: inline-size`만 까는 건
    **소비처 없는 speculative 인프라**다. 게다가 `contain: layout` 부작용(absolute 자손의
    컨테이닝 블록이 바뀜)이 실재한다.
    → **실패 사례가 나오면 그때 넣는다.** 재논의를 막으려 근거를 남긴다.

  **⛔ 수행 화면에는 R4를 적용하지 않았다.** 문서는 "12열 그리드도 동일 정책"이라 했지만,
  768px에서 `.visual`(span 5)=**285px**, `.details`(span 6)=**345px**로 홈 같은 뭉갬이 없다.
  고칠 문제가 없어서 두었다.

- [x] R-Step 5 — 완료 2026-08-18. **R6·R7 해결, R10은 측정 결과 수정 불필요.**

  **R7 터치 타깃** — 실측으로 확인:
  - `StepActions .subtle`(이전/건너뛰기) `min-height: 44px` → 실측 **44px** ✅
  - `PageHeader .back` `padding: 8px`→`10px` (10+24+10) → 실측 **44×44** ✅

  **R6 가로모드** — `@media (orientation: landscape) and (max-height: 500px)`에서 `.photo`를 `16/9`로.

  > ⚠️ 이 블록은 **반드시 768px 블록 뒤**에 와야 한다. 눕힌 폰은 폭이 768px를 넘어
  > 데스크톱 규칙(`aspect-ratio: 1`)이 먼저 걸리므로, 같은 특이도에서 **순서로 이겨야** 한다.

  **R10 320px — 재봤더니 문제가 없어서 고치지 않았다.** 이 문서는 카드가 132px라 좁다고
  적었지만, `--page-inline`(clamp 하한 16px) 도입 후 실측은 다르다:

  | 경로       | 가로 스크롤      | 카드 폭 | 내부 텍스트 넘침 |
  | ---------- | ---------------- | ------- | ---------------- |
  | `/`        | 없음 (305 = 305) | —       | —                |
  | `/scan`    | 없음             | 128.5px | **없음**         |
  | `/profile` | 없음             | 130px   | **없음**         |

  좁긴 해도 **잘리지 않는다.** 추측으로 `auto-fit` 같은 걸 넣지 않았다.

- [x] R-Step 6 — 다크 팔레트 — **완료 2026-08-18. R8 해결.**

  `globals.css`에 `@media (prefers-color-scheme: dark)` 블록(39토큰) + `color-scheme: light dark`,
  `layout.tsx` `themeColor`를 라이트/다크 2값으로, `ScanCard`의 `#ffffff`→`--surface-container-lowest`.
  값은 전부 이 문서의 톤 대응표에서 왔다(**창작 0건** — 세션이 39개 전수 대조).

  **hex 하드코딩은 2건뿐이었다.** `CameraCapture .stage`의 `#000`은 **카메라 영상 레터박스**라
  테마 표면이 아니다 → 토큰화 예외로 두고 사유 주석만 남겼다(다음 사람이 또 고민하지 않도록).

  ### ⭐ 이 문서의 전제가 틀렸다 — "`:root` 토큰만 바꾸면 전 화면이 전환된다"

  설계 절은 위험 요소를 **hex 하드코딩**으로만 봤다. 실제 원인은 달랐다:
  **컴포넌트가 `-fixed`(스킴 무관 고정) 토큰과 유동 토큰을 섞어 쓴다.**
  hex가 0건이어도 깨진다. 다크 실측에서 **6건**이 WCAG AA 미달이었다:

  | 요소                         | 다크 대비 | 원인                                                                               | 수정                              |
  | ---------------------------- | --------- | ---------------------------------------------------------------------------------- | --------------------------------- |
  | `profile .summaryValue`      | **1.00**  | 배경 `--secondary-container`와 전경 `-fixed-variant`가 **완전히 같은 hex**         | 전경 → `--on-secondary-container` |
  | `profile .tag`               | 1.53      | 동일 패턴                                                                          | 동일                              |
  | `RoutineCard .chip`          | 1.64      | 동일 패턴                                                                          | 동일                              |
  | `NextStepCard .thumb` 아이콘 | 1.74      | 배경·전경이 **같은 방향으로** 어두워짐(`--primary-container`는 컨테이너 배경 역할) | 전경 → `--primary`                |
  | `profile .sand`              | 2.14      | `-fixed` 짝인데 **50% 투명**이라 뒤 배경에 의존                                    | 배경 불투명화                     |
  | `profile .sage`              | 2.14      | 동일                                                                               | 동일                              |

  수정 후 재측정: 다크 7.29–11.99 / 라이트 4.56–7.29 — **전부 통과.**
  (`.summaryValue` 라이트 4.56은 20px/600 = WCAG 대형 텍스트라 기준이 **3:1**이다. 여유 충분.)

  **교훈 3가지 — 다음에 토큰을 만질 때 반드시 본다:**
  1. **`-fixed` 전경은 유동 배경과 짝지으면 안 된다.** 배경이 `--X-container`면 전경은
     `--on-X-container`다. `.concernCard`는 형제 `.overline`이 이미 올바른 짝이었는데
     `.summaryValue`만 어긋나 있었다 — 원래부터 버그였고 다크가 드러냈을 뿐이다.
  2. **`color-mix(… , transparent)`는 "고정"이 아니다.** 고정 토큰끼리 짝지어도
     반투명이면 합성 결과가 **뒤 배경에 의존**해 중간톤이 된다. 고정 짝은 **불투명하게** 쓴다.
  3. **반투명 합성 대비는 코드 판독으로 못 맞힌다.** 세션이 코드로 짚은 의심 4곳 중 2곳
     (`IngredientAlerts .conflict` 6.47, `ScanCard .description` 5.54)은 **멀쩡했고**,
     못 본 4곳이 실제 실패였다. **측정만이 답이다.**

  ⚠️ **라이트 모드도 바뀐 곳이 있다** — `.sand`·`.sage` 칩 배경이 50% 틴트 → 불투명이 되어
  채도가 올라간다(hue는 동일). 대비는 좋아지지만 시각적 변화가 있다.
