...
출력: `{ "concern_summary": string, "morning": [단계…], "evening": [단계…] }` — 단계 1개는 아래 구조다.

| 최상위 필드       | 타입     | → 프론트                | 비고                                                                                                         |
| ----------------- | -------- | ----------------------- | ------------------------------------------------------------------------------------------------------------ |
| `concern_summary` | `string` | 루틴 `name` 조합용 조각 | **2026-08-20 추가**(규칙 9) — wonder 요약 2–8자. 라우트가 2–20자 밖이면 `null` 강등, 이름은 요약 없이도 성립 |
