 ...
 *
 * ⭐ 이 프롬프트의 핵심 규칙은 **"확신이 없으면 빈 배열"** 이다.
 *    성분은 화면에 사실로 표시되고 **충돌 분석의 입력**이 된다 — 지어낸 성분 하나가
 *    "이 조합은 안전합니다"라는 잘못된 결론으로 이어진다. 그래서 **틀린 성분보다 빈
 *    성분이 낫다**. 빈 배열을 실패로 처리하지 마라(화면은 "성분표 사진을 올려 주세요"로
 *    이어진다). 이 규칙을 완화하면 이 서비스가 파는 신뢰가 통째로 무너진다.
 *
...
1-1. 이미지에 성분표가 보이지 않으면(제품 겉면만 찍힌 경우 등) ingredients를 빈 배열([])로 두세요. 이미지에서 읽을 수 있는 제품명이 있으면 product_name에만 반영하세요.
