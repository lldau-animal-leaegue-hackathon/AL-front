 ...
 *
 * 프롬프트가 **피부 고민 원문 + 보유 제품 전체**를 포함해 길어진다 — runner 가 stdin 으로
 * 넘기므로 argv 상한(32,767자)에 걸리지 않는다.
 * 그 전문이 세션 트랜스크립트에 그대로 남으므로 러너 기본값(cleanupSession: true)으로 지운다.
 * (옛 주석은 "도구를 안 쓰니 이미지가 안 남아 정리가 불필요하다"고 했는데 틀렸다 — 텍스트도 남는다.)
 */
